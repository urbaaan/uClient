...
if (store.get('RPC')) {
    discord = new(require('discord-rpc').Client)({
        transport: 'ipc'
    })
...
function INIT_CLIENT() {
    ...
    const SHORTCUT_CONFIG = {
        ...
        'F2': () => (typeof clipboard.readText === 'string') ? ((new URL(clipboard.readText()).hostname.includes('venge.io') && new URL(clipboard.readText()).pathname.split('#').length > 1) ? win.loadURL(clipboard.readText()) : getLink()) : getLink(),
        'F3': () => {
